...
<p class="txt">&nbsp;</p>';
	...
	public function jobseeker_email_verify($email_body,$row)
	{
    $email_body=replace_string('{JOBSEEKER_NAME}',$row['first_name'],$email_body);
		$email_body=replace_string('{OTPCODE}',$row['code'],$email_body);
		$email_body=replace_string('{SITE_URL}',base_url(),$email_body);
		return $email_body;
	}

	public function employer_email_verify($email_body,$row)
	{
		$email_body=replace_string('{EMPLOYER_NAME}',$row['first_name'],$email_body);
		$email_body=replace_string('{OTPCODE}',$row['code'],$email_body);
		$email_body=replace_string('{SITE_URL}',base_url(),$email_body);
	...
	public function jobseeker_interview_template($email_body,$row)
	{
		
        $email_body=replace_string('{INT_DATE}',$row['date'],$email_body);
		$email_body=replace_string('{INT_TIME}',$row['time'],$email_body);
		$email_body=replace_string('{INT_ADDRESS}',$row['address'],$email_body);
		$email_body=replace_string('{CONTACT}',$row['contact'],$email_body);
		return $email_body;
	}
